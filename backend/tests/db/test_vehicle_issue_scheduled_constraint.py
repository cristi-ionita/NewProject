...
def make_valid_user(**overrides) -> User:
    ...
    data = {
        ...
        "unique_code": f"ISSUE-SCH-{suffix}",
        "username": f"issue_sch_{suffix}",
...
@pytest.mark.asyncio
async def test_vehicle_issue_scheduled_requires_datetime(db_session):
    ...
    issue = VehicleIssue(
        ...
        status=VehicleIssueStatus.SCHEDULED,
        scheduled_for=None,
    )
...
@pytest.mark.asyncio
async def test_vehicle_issue_scheduled_with_datetime_is_valid(db_session):
